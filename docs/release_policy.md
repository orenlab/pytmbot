...
- `0.3.2`: exact release image, immutable
...
- Use `0.3.2` when you need a reproducible artifact tied to a specific release.
...
- Exact release tags such as `0.3.2` must never be republished with different contents.
- Weekly rebuilds must never move `0.3.2`.
- `latest` must always point to the newest supported stable line.

## Release Workflow

The release workflow publishes all of the following tags for `0.3.2`:
...
- For production fleets that prioritize predictable rollbacks, pin `0.3.2`.
