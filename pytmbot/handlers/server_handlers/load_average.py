...
def handle_load_average(message: Message, bot: TeleBot):
    ...
    try:
        ...
        if load_average is None:
            bot_logger.error(f"Failed at @{__name__}: Error occurred while getting load average")
            return bot.send_message(message.chat.id, text="Some error occurred. Please try again later(")

        with Compiler(template_name='b_load_average.jinja2', context=load_average, **emojis) as compiler:
