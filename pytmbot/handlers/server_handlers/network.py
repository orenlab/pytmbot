...
def handle_network(message: Message, bot: TeleBot):
    ...
    try:
        ...
        if network_statistics is None:
            bot_logger.error(f"Failed at @{__name__}: Error occurred while getting network statistics")
            return bot.send_message(
                message.chat.id, text="An error occurred while getting network statistics"
            )
