...
def _deletion_callback(result: DeletionResult) -> None:
    ...
    if result.status == DeletionStatus.SUCCESS:
        logger.debug(f"Message {result.message_id} successfully deleted for user {result.user_id}")
...
def handle_getmyid(message: Message, bot: TeleBot, auto_delete_delay: int = 30) -> Optional[Message]:
    """
    ...
    try:
        ...
        if deletion_result.status == DeletionStatus.LIMIT_EXCEEDED:
            ...
            logger.warning(
                f"Auto-deletion limit exceeded for user {user_id}. "
            )
