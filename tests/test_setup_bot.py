...
class TestConfigBuilder(unittest.TestCase):
    ...
    def test_create_dot_env(self, mock_prompt, mock_generate_salt, mock_set_file):
        ...
        sb.create_dot_env()
        mock_set_file.assert_called_once_with("pytmbot.yaml", expected_content.strip())
    ...
    def test_build_config_new_file(self, mock_create_dot_env, mock_has_file):
        with patch("click.secho") as mock_secho:
            ...
            mock_secho.assert_any_call(
                "*** Starting build default pytmbot.yaml ***", fg="green", bold=True
            )
            mock_secho.assert_any_call(
                "All done. Now you can build Docker image and run the bot.",
                blink=True,
                bg="blue",
                fg="white",
                bold=True,
    ...
    def test_build_config_existing_file(self, mock_has_file):
        with patch("click.secho") as mock_secho:
            sb.build_config()
            mock_secho.assert_any_call(
                "Looks like you already have pytmbot.yaml, if you need to reconfigure the bot, remove it.",
                blink=True,
                fg="yellow",
                bold=True,
            )
