...
def _extract_target_user_id(parsed: tuple[str, int, int, int]) -> int:
    return parsed[2]
...
def _render_from_parsed(
    ...
    action, image_index, target_user_id, page = parsed
    return render_image_extra_info(
        action=action,
...
def handle_image_extra_info(call: CallbackQuery, bot: TeleBot) -> None:
    return handle_image_details_callback(
        call=call,
        bot=bot,
        parse_callback=parse_image_extra_callback_data,
