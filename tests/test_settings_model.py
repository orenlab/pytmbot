...
def test_config_migrator_check_deprecation_emits_warning() -> None:
    with pytest.warns(DeprecationWarning):
        check_config_deprecation(None, "0.3.2")

    with pytest.warns(DeprecationWarning):
        check_config_deprecation("0.2.2", "0.3.2")


def test_config_migrator_migrate_config_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings_model_module, "get_app_version", lambda: "0.3.2")
    migrator = ConfigMigrator()

    legacy: _ConfigDict = {"bot_token": {"prod_token": ["token"]}}
    migrated_legacy = migrator.migrate_config(_as_object_dict(legacy))
    assert migrated_legacy["config_version"] == "0.3.2"

    outdated: _ConfigDict = {"config_version": "0.2.2"}
    migrated_outdated = migrator.migrate_config(_as_object_dict(outdated))
    assert migrated_outdated["config_version"] == "0.3.2"

    current: _ConfigDict = {"config_version": "0.3.2"}
    assert (
        migrator.migrate_config(_as_object_dict(current))["config_version"] == "0.3.2"
    )
...
def test_settings_model_migration_and_compatibility(
    ...
    monkeypatch.setattr(SettingsModel, "app_version", "0.3.2")
    payload = _base_config()

    settings = SettingsModel.model_validate(payload)
    assert settings.config_version == "0.3.2"
    ...
    assert upgraded.config_version == "0.3.2"
